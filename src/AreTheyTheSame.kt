...
fun comp(a: IntArray?, b: IntArray?): Boolean {
    ...
    return a.map { it.times(it) }.sortedByDescending { it }.zip(b.sortedByDescending { it }) { i, j ->
        i == j
    }.contains(false).not()
}
 ...
 */
fun compRefactored(a: IntArray?, b: IntArray?) = a != null && b != null && a.map { it * it }.sorted() == b.sorted()
